			...
			if (it->second.GetIp().IsSet())
			{
				...
				for (const auto c : it->second.GetModules())
				{
					...
					else if (it->second.UsesDHT())
					{
						...
						else if ((std::string::npos == it->second.GetTCMods().find(c)) != (std::string::npos == (g_Configure.Contains(g_Keys.tc.tcmodules) ? g_Configure.GetString(g_Keys.tc.tcmodules) : "").find(c)))
